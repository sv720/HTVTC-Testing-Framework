...
"""
final_HTVTC_SiLRTC is the same as final_HTVTC except that the tensor is no longer being completed with cross but with SiLRTC (from tenpy)
"""
"""
#DOESN'T WORK AS OF YET

def final_HTVTC_SiLRTC(ranges_dict, eval_func, metric, initial_known_fraction, assumed_rank=2, known_fraction_multiplier = 1, maxit_fctn = 1000, tol=1e-04, **kwargs):
     # Deal with kwargs that are not passed into tensor generation------------------------------------------------------
    kwargskeys = kwargs.keys()
    #The minimum resolution interval required for real-valued hyperparameter. For integers, the minimum is 1.
    min_interval = 1
    ...
    for cycle_num in range(max_completion_cycles):
        ...
        #print("DEBUG: sparse_tensor = \n ", sparse_tensor)
        completed_tensor= pyten_TC(sparse_tensor=sparse_tensor, function_name='SiLRTC', r=assumed_rank, tol=tol,maxiter=maxit_fctn)
        #print("DEBUG: completed_tensor = \n ", completed_tensor)
...
"""
final_HTVTC_FaLRTC is the same as final_HTVTC except that the tensor is no longer being completed with cross but with FaLRTC (from tenpy)
"""
def final_HTVTC_FaLRTC(ranges_dict, eval_func, metric, initial_known_fraction, assumed_rank=2, known_fraction_multiplier = 1, maxit_fctn = 1000, tol=1e-04, **kwargs):
     # Deal with kwargs that are not passed into tensor generation------------------------------------------------------
    kwargskeys = kwargs.keys()
    #The minimum resolution interval required for real-valued hyperparameter. For integers, the minimum is 1.
    min_interval = 1
    ...
    for cycle_num in range(max_completion_cycles):
        ...
        #print("DEBUG: sparse_tensor = \n ", sparse_tensor)
        completed_tensor= pyten_TC(sparse_tensor=sparse_tensor, function_name='FaLRTC', r=assumed_rank, tol=tol,maxiter=maxit_fctn)
        print("DEBUG: completed_tensor = \n ", completed_tensor)
...
"""
final_HTVTC_HaLRTC is the same as final_HTVTC except that the tensor is no longer being completed with cross but with FaLRTC (from tenpy)
"""
def final_HTVTC_HaLRTC(ranges_dict, eval_func, metric, initial_known_fraction, assumed_rank=2, known_fraction_multiplier = 1, maxit_fctn = 1000, tol=1e-04, **kwargs):
     # Deal with kwargs that are not passed into tensor generation------------------------------------------------------
    kwargskeys = kwargs.keys()
    #The minimum resolution interval required for real-valued hyperparameter. For integers, the minimum is 1.
    min_interval = 1
    ...
    for cycle_num in range(max_completion_cycles):
        ...
        #print("DEBUG: sparse_tensor = \n ", sparse_tensor)
        completed_tensor= pyten_TC(sparse_tensor=sparse_tensor, function_name='HaLRTC', r=assumed_rank, tol=tol,maxiter=maxit_fctn)
        print("DEBUG: completed_tensor = \n ", completed_tensor)
        ...
        completed_tensor[completed_tensor==0] = np.inf #assign zero values as these are not valid search points (proceed by elemination)
        bestValue = findBestValues(completed_tensor, smallest=True, number_of_values=1)
