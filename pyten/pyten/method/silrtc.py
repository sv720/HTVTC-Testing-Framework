...
def pro_to_trace_norm(z, tau):
    ...
    if m > 2 * n:
        ...
        print("DEBUG: z.T.shape = ", z.T.shape )
        [U, Sigma2, V] = np.linalg.svd(np.dot(z, z.T))
        S = np.sqrt(Sigma2)
        tol = np.max(z.shape) * (2 ** int(math.log(max(S), 2))) * 2.2204 * 1E-16
        k = np.sum(S > max(tol, tau))
    ...
    print("DEBUG: S = ", S)
    print("DEBUG: tau = ", tau)
    print("DEBUG: S.flatten() > tau = ", S.flatten() > tau)
    k = sum(S.flatten() > tau)
    #print("DEBUG: type(U) = ", type(U))
    #print("DEBUG: U = \n", U)
    #print("DEBUG: k = ", k)
    #print("DEBUG: U[:, 0:k] = ", U[:, 0:k[0]])
    #print("DEBUG: type(S[0:k]) = ", type(S[0:k]))
    #print("DEBUG: type(tau) = ", type(tau))
    #print("DEBUG: type(V[0:k, :]) = ", type(V[0:k, :]) )
    if k > 0:
        X = np.dot(U[:, 0:k], np.dot(np.diag(S[0:k] - tau), V[0:k, :]))
    else: #my addition to handle case with number of singular values above the threshold is 0
        X = np.zeros_like(z)
    return X, n, Sigma2
...
def silrtc(x, omega=None, alpha=None, gamma=None, max_iter=100, epsilon=1e-5, printitn=100):
    ...
    for k in range(max_iter):
        ...
        for i in range(N):
            ...
            print("DEBUG: type(temp) = ", type(temp))
            print("DEBUG: dir(temp)", dir(temp))
            print("DEBUG: temp.totensor().data.shape = ", temp.totensor().data.shape)
            [temp1, tempn, tempSigma2] = pro_to_trace_norm(temp.totensor().data, tau[i])
            temp.data = temp1
            print("DEBUG: type(temp.totensor().data) = ", type(temp.totensor().data))
