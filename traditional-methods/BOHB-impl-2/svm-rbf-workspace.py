...
class MyWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        ...
        func = crossValidationFunctionGenerator(data_split, algorithm='svm-rbf', task=task, budget_type='samples', budget_fraction=budget)
    ...
    def get_configspace():
        ...
        C = CSH.UniformFloatHyperparameter('C', lower=0.05, upper=5.0)
        gamma = CSH.UniformFloatHyperparameter('gamma', lower=0.05, upper=5.0)
        cs.add_hyperparameters([C, gamma])

        return cs

#Setup nameserver
NS = hpns.NameServer(run_id='svm-rbf', host='127.0.0.1', port=None)
NS.start()

#Start a worker
w = MyWorker(sleep_interval = 0, nameserver='127.0.0.1',run_id='svm-rbf', logger=logObj)
...
bohb = BOHB(  configspace = w.get_configspace(),
              run_id = 'svm-rbf', nameserver='127.0.0.1',
